...
public class GauzatuEragiketaBDBlackTest {
	...
	public void negativeTest3() {
		...
		try {
			...
			testDA.open();
			if (!testDA.existDriver(username)) {
				driver = testDA.createDriver(username, pass); // even this fuction probably shouldn't accept empty
																// strings...
				driverCreated = true;
			} else
				driver = sut.getDriver(username);
	...
	public void negativeTest8() {
		...
		try {
			// Add testuser to database and save User object as "driver"
			/*
			 * testDA.open(); if (!testDA.existDriver(username)) { driver =
			 * testDA.createDriver(username,pass); driverCreated = true; } else driver =
			 * sut.getDriver(username); testDA.close();
			 */
			sut.open();
			...
			// Cleanup
			/*
			 * testDA.open(); if (driverCreated) testDA.removeDriver(username);
			 * testDA.close();
			 */
			sut.close();
		}
	...
	public void negativeTest10() {
		...
		try {
			...
			// Uncaught Exception is thrown, program failed. No checks are done for this
			// error.
			fail();
		} catch (Exception e) {
			e.printStackTrace();
